...
const PopupWidget = () => {
  ...
  const handleOnSubmit = (event) => {
    ...
    axios({
      method: 'POST',
      url: 'https://formbold.com/s/35XB2',
      data: inputs,
    })
      .then((r) => {
        if (r.status === 201) {
    ...
    <div>
      {showModal && (
        <div className="fixed z-10 inset-0 flex items-center justify-center px-4 sm:px-0">
          <div className="bg-white w-full sm:w-1/3 p-6 rounded-lg shadow-lg">
          ...
          <>
            ...
            >
              <Disclosure.Panel className=" flex flex-col  overflow-hidden left-0 h-full w-full sm:w-[350px] min-h-[250px] sm:h-[600px] border border-gray-300 bg-white shadow-2xl rounded-md sm:max-h-[calc(100vh-120px)]">
                <div className="flex flex-col items-center justify-center h-32 p-5 bg-blue-500">
                  <h3 className="text-lg text-white">How can we help?</h3>
                  <p className="text-white opacity-50">
                    We usually respond in a few hours
                  </p>
                </div>
                <div className="flex-grow h-full p-6 overflow-auto bg-white-500 ">
                  {!isSubmitSuccessful && (
